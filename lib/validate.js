...
function validatePluginItem(plugin, index) {
  ...
  // 验证插件名称是否在支持的列表中, 如果存在 customPlugin 则不进行验证
...
module.exports = function validateConfig(config) {
  ...
  // maxWorkerNum 检查（如果存在，必须是大于1，且小于8的整数）
  if ('maxWorkerNum' in config) {
    if (typeof config.maxWorkerNum !== 'number' || config.maxWorkerNum < 1 || config.maxWorkerNum > 8) {
      throw new Error('maxWorkerNum must be a number between 1 and 8');
    }
  }
  // 验证数组存在性
