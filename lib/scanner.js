...
class Scanner {
  ...
  spawnWorkerProcess(source, resultDir) {
    return fork(path.resolve(__dirname, 'worker.js'), [JSON.stringify(source), resultDir, JSON.stringify(this.plugins)]);
  }
  ...
  // 扫描主入口
  async scan() {
    this.logger.info(`scan started, total tasks: ${this.sources.length}`);
