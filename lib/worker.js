...
class Worker {
  ...
  initializeHooks() {
    this.hooks = {
      pre: new AsyncSeriesHook(['context']),          // 前置hook
      ...
      after: new AsyncSeriesHook(['context']),        // 后置hook
    };
  }
  ...
  async runPlugin() {
    ...
    try {
      ...
      await this.hooks.pre.promise(context);
      ...
      await this.hooks.after.promise(context);
  ...
  // 创建上下文
  createContext() {
    const context = { ...this.source, resultDir: this.resultDir };
